...
function createStation(payload) {
  ...
  try {
    ...
    if (context.viewer.testMode) {
      ...
      // 不在「人員職務配置」自動新增駐站管理員列：所選管理員已存在於系統中，
      // 重複寫入會造成資料重複，且會把管理員的主管錯設為其自己（Issue #7）。
      // 駐站與管理員的從屬關係改由「組織架構樹」的主管欄位（managerEmail/managerName）維護。
...
function deleteStation(payload) {
  ...
  try {
    ...
    if (context.viewer.testMode) {
      ...
      // 刪除前先封存整列（含 I 欄臨調摘要）到歷史封存表，保留稽核軌跡（Issue #8）。
      // 封存失敗會中止刪除（不會吞例外），確保不會在無歷史紀錄的情況下硬刪除資料。
      archiveStationRowsBeforeDelete_(source, stationCode, {
        operator: viewerEmail,
        deletedAt: formatTimestamp_(new Date())
      });
